	...
	if (!function_exists('sga_register_gallery_type') && !($_REQUEST['plugin']=='simplest-gallery/simplest-gallery.php' && $_REQUEST['action']=='activate')) {
		...
		$result = sga_register_gallery_type(
							...
							array('fancybox'=>$urlpath . '/fancybox/jquery.fancybox-1.3.4.css',
								'fancybox-override'=>$urlpath . '/fbg-override.css')			
						);
...
function sgli_header() {
	return "<!-- Lighbox module for Simplest Gallery -->\n";
}
...
function sgli_render($images,$thumbs,$post_id=NULL,$gall_id=NULL,$attrs,$gallery_type='lightbox') {
    ...
    $columns=3;
    if (is_array($attrs) && intval($attrs['columns'])) $columns = $attrs['columns'];
...
';
	...
	for ($i=0;$i<count($thumbs);$i++) {
		...
		<a class="fancybox" href="'.$image[0].'"'.(($gallery_type == 'lightbox_labeled')?' title="'.htmlspecialchars($thumb[5]).'"':'').' rel="gallery-'.$gallid.'"><img width="'.$thumb[1].'" height="'.$thumb[2].'" class="attachment-thumbnail" alt="'.htmlspecialchars($thumb[5]).'" src="'.$thumb[0].'" /></a></dt>';
		if ($gallery_type == 'lightbox_labeled') {	// Add labels
